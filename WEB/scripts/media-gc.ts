...
import { openLocalDb, createLocalDb, DATA_DIR } from "../src/db/local";
...
const descriptions = db.select({ description: tasks.description }).from(tasks).all();
...
for (const name of readdirSync(UPLOAD_DIR)) {
  ...
  const age = now - statSync(join(UPLOAD_DIR, name)).mtimeMs;
